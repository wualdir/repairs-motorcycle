...
export class CreateUserDto{
    ...
    static createUdto(objetc:{[key:string]:any}):[string?,CreateUserDto?]{
        ...
        if(!RegularExpre.password.test(password)) return ['The password must be at least 10 character long and contain  at least one uppercase letter ,one lowercase letter, one number and one special character']
